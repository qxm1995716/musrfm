...
# this function is used to read the rhos raster and mask raster, without dem
# (same as above, just do not have elev_raster)
def load_rhos(path, mask_path, nan_filter=True):
    r"""
    :param path: the path for the rhos raster -> string
    :param mask_path: the path for the region mask for interpolation -> string
    :param nan_filter: whether perform the interpolation -> bool
    :return:
    rhos_raster: the obtained rhos tensor -> np::ndarray -> [h w c]
    extent: the extent of rhos(elev) tif -> gdal::GeoTransform
    proj: the proj of rhos(elev) tif -> gdal::Projection
    """
    rhos = gdal.Open(path)
    ...
    handler.GetRasterBand(1).SetNoDataValue(-111111)
    # re-project the mask to align with rhos
    gdal.ReprojectImage(mask, handler, mask_proj, proj, gdalconst.GRA_Bilinear)
    mask_raster = handler.GetRasterBand(1).ReadAsArray()
    mask_ndv = handler.GetRasterBand(1).GetNoDataValue()
    # bool_mask is for determine the area where is needed to perform interpolation (improve efficiency).
    ...
    return rhos_raster, extent, proj


'''
def RMDB_Reader(raster_path, region_mask_path, nan_filter=False):
    rhos, mask, extent, proj = load_rhos_gt(raster_path, region_mask_path, mask_path=region_mask_path,
                                            nan_filter=nan_filter)
    update_bathy = rhos[:, :, -1]
    update_dt = rhos[:, :, -2]
    RHOS = rhos[:, :, :-3]
    update_bathy[mask == -111111] = -111111

    return RHOS, update_bathy, update_dt, extent, proj
'''
...
# the pre-process for obtain data without dem data, in this process there is no dem guided correction
def rhos_wd_preprocess(raster_path, mask_path, thres, is_output=True, PATH=None):
    r"""
    :param raster_path: the path for the rhos raster -> string
    :param mask_path: the path for the mask raster -> string
    :param thres: the threshold value for the b8mask function
    ...
    :return:
    """
    rhos, extent, proj = load_rhos(raster_path, mask_path)
    # get the file name of raster, for further output
    update_rhos_path = raster_path.split('.')[0]
    # the valid_rhos is used to construct a bool mask for invalid pixels, if there is an abnormal value in one pixel,
    # then this pixel will be marked as invalid
    ...
    # perform binary threshold algorithm to obtain b8mask and b8mask_dt
    b8mask, b8mask_dt = B8mask(rhos, thres)
    # get the DtCM of b8mask
    reverse_b8mask = (1 - b8mask).astype(np.uint8)
    update_dt = cv2.distanceTransform(reverse_b8mask, distanceType=cv2.DIST_L2, maskSize=5)
    print("The data have processed, with no add dem included.")

    # if these processed raster needed to output, if output, it will add with "_RMDB_MAXE_XXXXX(max elevation).tif"
    if is_output:
        if PATH is None:
            update_rhos_path = update_rhos_path + '_RMD.tif'
        else:
            if not os.path.exists(PATH):
                os.mkdir(PATH)
            update_rhos_path = PATH + '/' + update_rhos_path.split('/')[-1] + '_RMD.tif'
        # this data contain [R: rhos M: mask, D: DtCM, B: Bathymetry] -> RMDB
        raster_output(update_rhos_path, [rhos, b8mask, update_dt], extent=extent, proj=proj)

    else:
        return rhos, b8mask, update_dt, extent, proj
